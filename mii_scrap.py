from bs4 import BeautifulSoup
import requests
import os

git = 'https://github.com/EchoOfEden/obrazki/raw/main/products/'
# https://mi-home.pl/wszystkie-produkty?p=8
#"Product ID";Obraz;Nazwa;Indeks;Kategoria;"Cena (brutto)";Ilość;Widoczność;Opis;"Meta-opis";Tagi;Kombinacje;"Pozwól zamawiać";EAN;Atrybuty
class produkt:
        def __init__(self,ID,obraz,nazwa,sku,kategoria,cena,opis,tagi,kombinacje,EAN,atrybuty):
            self.ID = ID
            self.obraz = obraz
            self.nazwa = nazwa
            self.sku = sku
            self.kategoria = kategoria
            self.cena = cena
            self.ilosc = 99
            self.widocznosc = 'both'
            self.opis = opis
            self.tagi = tagi
            self.kombinacje = kombinacje
            self.pozwolzamawiac = 1
            self.EAN = EAN
            self.atrybuty = atrybuty
            
prodList = []

#download pictures
def img_download(url, index):
    img_name = url.split('/')[-1]
    img_file = requests.get(url).content
    if not os.path.exists('products'):
        os.mkdir('products')
        print("Directory Created ")
    if not os.path.exists('products\\'+str(index)):
        os.mkdir('products\\'+str(index))
        print("Directory " , index ,  " Created ")
    with open('products\\'+ str(index) +'\\' + img_name, 'wb') as handler:
        handler.write(img_file)

def appendPictures(soup, productID, getPictures):
    obraz = str(prodList[productID-2].obraz)
    for j in soup.findAll('script',{'type':'application/ld+json'}):
        if 'ImageGallery' in j.text:
            images = j.text.split('"ImageGallery","image":"')[1].split('"}   ')[0].replace('\/','/').split(',')
            for x in images:
                if getPictures:
                    img_download(x, productID)
                if x == images[-1]:
                    if not x.split('/')[-1] in obraz: obraz += git + str(productID) + '/' + x.split('/')[-1]
                    break
                if not x.split('/')[-1] in obraz:  obraz += git + str(productID) + '/' + x.split('/')[-1] +','
            break
        #wersja z wariantami
        elif j == soup.findAll('script',{'type':'application/ld+json'})[-1]:
            for k in soup.findAll('script',{'type':'text/x-magento-init'}):
                if '[data-role=swatch-options]' in k.text:
                    id = soup.find('input',{'name':'swatchOptions'})['value']
                    if 'Colour' in id:
                        id = id.split('"Colour":"')[1].split('"')[0]
                    elif 'color' in id:
                        id = id.split('color":"')[1].split('"')[0]
                    elif 'Color_name":"' in id:
                        id = id.split('Color_name":"')[1].split('"')[0]
                    else:
                        image = soup.find('meta',{'property':'og:image'})['content']
                        if getPictures:
                                img_download(image, productID)
                        if not image.split('/')[-1] in obraz: obraz += git + str(productID) + '/' + image.split('/')[-1]                        
                        break
            
                    id = k.text.split(id)[1].split('],"saleableProducts"')[0].split('[')[1].replace('"','').split(',')[0]
                    images = k.text.split('"images":')[1].split(str(id))[1].split('"videoUrl":null}]')[0].replace('\/','/').split('","full":"')
                    images.pop(0)

                    for x in images:
                        image = x.split('","caption"')[0]
                        if getPictures:
                            img_download(image, productID)
                        if x == images[-1]:
                            if not image.split('/')[-1] in obraz: obraz += git + str(productID) + '/' + image.split('/')[-1]
                            break
                        if not image.split('/')[-1] in obraz: obraz += git + str(productID) + '/' + image.split('/')[-1]+','
                    break

                elif k == soup.findAll('script',{'type':'text/x-magento-init'})[-1]:
                    image = soup.find('meta',{'property':'og:image'})['description']
                    if getPictures:
                            img_download(image, productID)
                    if not image.split('/')[-1] in obraz: obraz += git + str(productID) + '/' + image.split('/')[-1]
    prodList[productID-2].obraz = obraz

#wyciaganie danych ze strony do zmiennych
def getData(soup, productID, categoryID, getPictures):
    obraz = ''
    atrybuty = ''
    EAN = ''
    nazwa = soup.find('div', {'class':'product-info-main block'}).find('h1',{'class':'page-title'}).text.strip().replace('"','""')
    cena = soup.find('div', {'class':'product-info-main block'}).find('span',{'class':'price'}).text.strip().replace(' zł','').replace(' ','').replace(',','.')
    opis = soup.find('div', {'itemprop': 'description'}).text.strip().replace('"','""').replace('\n',' ')
    tagi = soup.find('meta',{'name':'keywords'})['content'].replace('\r','').replace('\n','').replace('"','""')
    sku = soup.find('form',{'id':'product_addtocart_form'})['data-product-sku']
    customizable = 0


    #images
    for j in soup.findAll('script',{'type':'application/ld+json'}):
        if 'ImageGallery' in j.text:
            images = j.text.split('"ImageGallery","image":"')[1].split('"}   ')[0].replace('\/','/').split(',')
            catimage = images[0]
            for x in images:
                if getPictures:
                    img_download(x, productID)
                if x == images[-1]:
                    obraz += git + str(productID) + '/' + x.split('/')[-1]
                    break
                obraz += git + str(productID) + '/' + x.split('/')[-1] +','
            break
        #wersja z wariantami
        elif j == soup.findAll('script',{'type':'application/ld+json'})[-1]:
            for k in soup.findAll('script',{'type':'text/x-magento-init'}):
                if '[data-role=swatch-options]' in k.text:
                    id = soup.find('input',{'name':'swatchOptions'})['value']
                    if 'Colour' in id:
                        id = id.split('"Colour":"')[1].split('"')[0]
                    elif 'color' in id:
                        id = id.split('color":"')[1].split('"')[0]
                    elif 'Color_name":"' in id:
                        id = id.split('Color_name":"')[1].split('"')[0]
                    else:
                        image = soup.find('meta',{'property':'og:image'})['content']
                        catimage = image
                        if getPictures:
                                img_download(image, productID)
                        obraz += git + str(productID) + '/' + image.split('/')[-1]                       
                        break
            
                    id = k.text.split(id)[1].split('],"saleableProducts"')[0].split('[')[1].replace('"','').split(',')[0]
                    images = k.text.split('"images":')[1].split(str(id))[1].split('"videoUrl":null}]')[0].replace('\/','/').split('","full":"')
                    images.pop(0)

                    catimage = images[0].split('","caption"')[0]
                    for x in images:
                        image = x.split('","caption"')[0]
                        if getPictures:
                            img_download(image, productID)
                        if x == images[-1]:
                            obraz += git + str(productID) + '/' + image.split('/')[-1]
                            customizable = 1
                            break
                        obraz += git + str(productID) + '/' + image.split('/')[-1]+','
                    break


                elif k == soup.findAll('script',{'type':'text/x-magento-init'})[-1]:
                    image = soup.find('meta',{'property':'og:image'})['description']
                    if getPictures:
                            img_download(image, productID)
                    obraz += git + str(productID) + '/' + image.split('/')[-1]

    #category
    #"Category ID";Aktywny;Nazwa;Kategoria-rodzic;Głowna
    script = soup.findAll('script',{'type':'text/x-magento-init'})
    word = 'product_breadcrumbs'
    for j in script:
        if word in j.text:
            category = j.text.strip().split('"name":"')[1].split('"')[0].replace('\\u0105','ą')
            subcategory = soup.find('div', {'class':'product-info-main block'}).find('h2',{'class':'mi-product-type'}).text.strip()
            kategoria = subcategory
            c = open("MIIcategory.csv", "r", encoding="utf-8")
            if not category in c.read():
                c.close()
                c = open("MIIcategory.csv", "a", encoding="utf-8")
                c.write(str(categoryID) + ';1;' +category + ';"Wszystkie produkty";0;' + git + str(productID) + '/' + catimage + '\n')
                c.close()
                categoryID += 1
            c = open("MIIcategory.csv", "r", encoding="utf-8")
            if not subcategory in c.read():
                c.close()
                c = open("MIIcategory.csv", "a", encoding="utf-8")
                c.write(str(categoryID) + ';1;' +subcategory + ';' + category + ";0;" + git + str(productID) + '/' + catimage + '\n')
                categoryID += 1
            break

    #atrybuty - ogólnie jak jest swatch na stronie produktu to są dostępny różne kolory/wymiary/itp
    swatch = soup.findAll('div',{'class':'swatch-opt'})
                
    #Cechy, z tego można wyciągnąć kolory same kolory to ostatni tr w tabelce additional-attributes
    techniczne = soup.find('table',{'class':'data table additional-attributes'}).findAll('tr')
    for x in techniczne:
        cecha = x.text.split('\n\n')[0].replace('\n','') 
        wartosc =  x.text.split('\n\n')[1]
        #print(cecha)
        if cecha == 'EAN':
            EAN = wartosc
        if len(swatch) != 0:
            customizable = 1
            if x == techniczne[-1]:
                if cecha == 'Długość kabla':
                    atrybuty = 'Długość kabla:'+ wartosc
                break
            if cecha == 'Długość kabla':
                atrybuty = 'Długość kabla:'+ wartosc
    prodList.append(produkt(productID,obraz,nazwa,sku,kategoria,cena,opis,tagi,customizable,EAN,atrybuty))
    return categoryID

#tworzy tablicę obiektów złożonych z danych ze strony a potem klei z 
def info_saving(products, getPictures):
    productID = 1
    categoryID = 10
    
    f = open("MIIproducts.csv", "w", encoding="utf-8")
    c = open("MIIcategory.csv", "w", encoding="utf-8")
    f.write('"Product ID";Obraz;Nazwa;Indeks;Kategoria;"Cena (brutto)";Ilość;Widoczność;Opis;"Opis meta";Tagi;Konfigurowalny;"W sprzedaży";"kod EAN";Cechy\n')
    c.write('"Category ID";Aktywny;Nazwa;Kategoria-rodzic;Głowna;Zdjęcie\n')
    c.close()
    
    for i in products:
        if productID <= 500:
            try:
                soup = BeautifulSoup(requests.get(i).content, 'html.parser')
                sku = soup.find('form',{'id':'product_addtocart_form'})['data-product-sku']

                if len(prodList) > 1 and sku == prodList[productID-2].sku :
                    appendPictures(soup, productID, getPictures)
                else:
                    categoryID = getData(soup, productID, categoryID, getPictures)
                    productID += 1

                if productID % 50 == 0:
                    print(str(productID/5) + "% done")

            except:
                print('Error on: ' + i)
    
    for k in prodList:
        f.write( str(k.ID)+';'+k.obraz+';'+k.nazwa+';'+k.sku+';'+k.kategoria+';'+str(k.cena)+';'+str(k.ilosc)+';'+k.widocznosc+';'+k.opis+';'+k.opis+';'+k.tagi+';'+str(k.kombinacje)+';'+str(k.pozwolzamawiac)+';'+str(k.EAN)+';'+k.atrybuty+'\n')

    f.close()

#po prostu zbiera linki do produktów
def link_scraping(url):
    products = []
    for i in range(25):
        soup = BeautifulSoup(requests.get(url + str(i+1)).content, 'html.parser')
        link = soup.findAll('a', {'class': 'product photo product-item-photo'})
        for j in link:
            if j.has_attr('href') and str(j['href']) != 'https://mi-home.pl/hybrydowe-szklo-do-mi-max-3':
                products.append(str(j['href']))
        print('Page ' + str(i+1) + ' links scrapped')

    return products
    
def main():
    url = "https://mi-home.pl/wszystkie-produkty?p="
    products = link_scraping(url)
    productsDummy = ['https://mi-home.pl/lexar-microsdxc-uhs-i-card-64gb-adapter']
    print("MAKING CSV FILE")
    info_saving(products, False)
    print("DONE!")

if __name__ == "__main__":
    main()
